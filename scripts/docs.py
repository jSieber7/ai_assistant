...
def main():
    ...
    if command == "serve":
        ...
        cmd = ["uv", "run", "mkdocs", "serve"] + merge_arguments(default_args, user_args)
        run_cmd(cmd, "Starting docs server")
    elif command == "build":
        default_args = []
        cmd = ["uv", "run", "mkdocs", "build"] + merge_arguments(default_args, user_args)
        run_cmd(cmd, "Building docs")
    elif command == "deploy":
        default_args = ["--force"]
        cmd = ["uv", "run", "mkdocs", "gh-deploy"] + merge_arguments(default_args, user_args)
